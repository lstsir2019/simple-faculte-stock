...
public interface StockDao extends JpaRepository<Stock, Long> {
    ...
    public final String QUERY_ALL_STOCK_GLOBAL = "SELECT NEW com.faculte.simplefacultestock.domain.model.service.dto.StockGlobalDTO(s.referenceCommande,s.referenceProduit,s.magasin.reference,SUM(s.qte)) FROM Stock s group by s.referenceCommande,s.referenceProduit,s.magasin.reference ORDER BY s.referenceCommande,s.referenceProduit";

    public final String QUERY_ONE_STOCK_GLOBAL = "SELECT NEW com.faculte.simplefacultestock.domain.model.service.dto.StockGlobalDTO(s.referenceCommande,s.referenceProduit,s.magasin.reference,SUM(s.qte)) FROM Stock s WHERE s.referenceCommande =?1 AND s.referenceProduit =?2 GROUP BY s.magasin.reference";

    @Query(QUERY_ALL_STOCK_GLOBAL)
    public List<StockGlobalDTO> findAllByCommande();
